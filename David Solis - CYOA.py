...
pistol = CommonW('Pistol', 'A semi-automatic pistol with 16 bullets in each magazine.', 100, None, 10,
                 'Can only shoot once at a time.')

shotgun = CommonW('Shotgun', 'A shotgun that does more damage than the average common weapon.', 130, None, 15,
                  'Can shoot multiple people wih on shot.')

grenade = CommonW('Grenade', 'A grenade that does an instant kill but only if person is close to grenade.', 150, None,
...
all_armor = [diamond_clothing, emerald_clothing, platinum_clothing, iron_clothing, metal_clothing, wood_clothing,
             bronze_clothing, chain_clothing, cotton_clothing, infinity_gauntlet_armor, ]
all_weapons = [ray_gun, wonder_waffle, ray_gun_mark11, rocket_launcher, assault_rifle, revolver, grenade, shotgun,
               infinity_gauntlet_weapon, pistol]
...
while True:
    ...
    if command == 'attack':
        ...
        if attack_description == 'true':
            ...
            if current_node.is_shrek_room is None:
                print("Shrek is not in the room.")
            elif current_node.is_shrek_room is shrek:
                ...
                if shrek.health >= 0:
                    ...
                    if inventory.armor in all_armor and inventory.armor.armor_amount > 0:
                        ...
                        if take_damage_or_no == 1:
                            ...
                            print("Shrek attacked back but you were able to shield it with your mighty armor.")
    if command == 'shrek':
        if shrek_actual is current_node:
            print("Shrek is in your room.")
        elif shrek_actual is not current_node:
            print("Shrek is in %s." % shrek_actual.name)
    if command == 'take':
    ...
    if command == 'snap':
        if inventory.armor is infinity_gauntlet_armor and inventory.weapon is infinity_gauntlet_weapon:
            print("Half the universe has died, but now your gauntlet is broken and unusable.")
